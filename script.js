...
const maleData = document.getElementsByClassName("male-data")[0];
...
function handleSearch(event) {
    const searchValue = event.previousElementSibling.value;
    const arr = students.filter((element) => {
        const str = element.first_name.toLowerCase() + " " + element.last_name.toLowerCase() + " " + element.email.toLowerCase();
...
for (let i = 0; i < sortButtons.length; i++) {
    sortButtons[i].addEventListener("click", handleSort);
}

function handleSort(event) {
    switch (event.target.name) {
        case "asc": students.sort((a, b) => {
            const fullNameA = a.first_name + " " + a.last_name;
            const fullNameB = b.first_name + " " + b.last_name;
            return fullNameA.localeCompare(fullNameB);
        });
            break;
        case "desc": students.sort((a, b) => {
            const fullNameA = a.first_name + " " + a.last_name;
            const fullNameB = b.first_name + " " + b.last_name;
            return fullNameB.localeCompare(fullNameA);
        });
            break;
        case "marks": students.sort((a, b) => a.marks - b.marks);
            break;
        case "pass": const arr = students.filter(element => element.passing);
            showData(arr);
            return;
        case "class": students.sort((a, b) => a.class - b.class);
            break;
        case "gender": const femalesArray = students.filter(element => element.gender == "Female");
            showData(femalesArray);
            const malesArray = students.filter(element => element.gender == "Male");
            showMaleData(malesArray);
            return;
    }
    showData(students);
}

function showMaleData(arr) {
    maleData.id = "show";
    const maleDataTBody = document.querySelector(".male-data > tbody");
    for (let i = 0; i < arr.length; i++) {
        const student = arr[i];
        const tr = document.createElement("tr");
        tr.innerHTML = `
